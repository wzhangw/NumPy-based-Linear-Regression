...
def main(debug_mode=True, loss_plot_mode=True):
    X, y = make_regression(n_samples=100, n_features=10, noise = 3)
    ...
    fitting_successs = regressor.fit(X=X_train, Y_actual=y_train, decay_rate=0.0, early_stopping_point=5000, convergence_tolerance = 10, batch_size=1, debug_mode=debug_mode, loss_plot_mode=loss_plot_mode)
    if fitting_successs:
